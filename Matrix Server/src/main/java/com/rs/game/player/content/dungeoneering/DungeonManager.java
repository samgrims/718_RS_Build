...
public class DungeonManager {
	...
	public void loadRoom(final RoomReference reference) {
		...
		visibleMap[reference.getX()][reference.getY()] = new VisibleRoom(
				party.getFloorType(), room.getRoom());
	...
	public void linkPartyToDungeon() {
		...
		for (DungeonPartyPlayer dplayer : party.getTeam()) {
			...
			player.getPackets().sendGameMessage(
					"Floor " + party.getFloor() + " Complexity "
							+ party.getComplexity() + " (Full)");
			player.getPackets().sendGameMessage("Dungeon Size: " + "Small");
			player.getPackets().sendGameMessage(
					"Party Size:Dificulty " + party.getTeam().size() + ":"
							+ party.getTeam().size());
			player.getPackets().sendGameMessage("");
		}
	}
		...
		CoresManager.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					dungeon = new Dungeon(party.getFloorType(), party
							.getComplexity(), size);
