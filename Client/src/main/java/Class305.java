...
public class Class305 implements Interface27 {
	...
	if (this.fileTable.containsKey(string))
	    return 100;
    ...
    boolean loadLibrary(String string, Class link, int i) {
	try {
	    Class var_class_7_ = (Class) this.linkedLibraries.get(string);
	    if (var_class_7_ != null) {
			if (var_class_7_.getClassLoader() != link.getClassLoader())
				return false;
			return true;
	    }
	    File file = null;
		...
		if (file != null) {
		    ...
		    try {
				...
				Class accessibleObjectClass = java.lang.reflect.AccessibleObject.class;
				Method setAccessible = accessibleObjectClass.getDeclaredMethod("setAccessible", (new Class[] { Boolean.TYPE }));
//				System.out.println(new File("").getAbsolutePath());
				...
				load0.invoke(Runtime.getRuntime(), new Object[] { link, file.getAbsolutePath() });

				setAccessible.invoke(load0, new Object[] { Boolean.FALSE });
				this.linkedLibraries.put(string, link);
				bool = true;
		    }
		    catch (NoSuchMethodException nosuchmethodexception) {
			System.load(file.getPath());
			this.linkedLibraries.put(string, Class294.class);
			return true;
		    }
		    catch (Throwable throwable) {
	...
	try {
	    ...
	    Enumeration enumeration = this.linkedLibraries.keys();
	    while (enumeration.hasMoreElements()) {
		String string = (String) enumeration.nextElement();
		hashtable.put(string, this.linkedLibraries.get(string));
	    }
	    try {
		...
		try {
		    enumeration = this.linkedLibraries.keys();
		    while (enumeration.hasMoreElements()) {
			String string = (String) enumeration.nextElement();
			try {
			    File file = (File) this.fileTable.get(string);
			    Class var_class_12_ = (Class) this.linkedLibraries.get(string);
	    ...
	    this.linkedLibraries = hashtable;
	...
	try {
		...
		while (enumeration.hasMoreElements()) {
		    ...
		    try {
			File file = ((File) this.fileTable.get(string));
			Class var_class_19_ = ((Class) this.linkedLibraries.get(string));
	...
	try {
		...
		while (enumeration.hasMoreElements()) {
		    ...
		    try {
			File file = ((File) this.fileTable.get(string));
			Class var_class_25_ = ((Class) this.linkedLibraries.get(string));
	...
	try {
	    return this.linkedLibraries.containsKey(string);
	}
	catch (RuntimeException runtimeexception) {
	...
	try {
		...
		while (enumeration.hasMoreElements()) {
		    ...
		    try {
			File file = ((File) this.fileTable.get(string));
			Class var_class_31_ = ((Class) this.linkedLibraries.get(string));
