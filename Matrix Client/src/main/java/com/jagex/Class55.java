		...
		try {
			if (strings.length > 1) {
				for (int i_1_ = 0; i_1_ < strings.length; i_1_++) {
					if (strings[i_1_].startsWith("pause")) {
						...
						Class255.method2435(new StringBuilder().append(
								"Pausing for ").append(i_2_).append(
								" seconds...").toString(), 357563950);
				...
				Class8.aString101 = new StringBuilder().append(
						Class8.aString101).append(strings[0]).toString();
				Class8.anInt99 += strings[0].length() * 1914783065;
			}
		} catch (RuntimeException runtimeexception) {
			throw Class346.method4175(runtimeexception, new StringBuilder()
					.append("ce.d(").append(')').toString());
		}
	}
		...
		try {
			if (-1217082313 * Class8.anInt105 > 0) {
				...
				for (int i_4_ = 0; i_4_ < Class8.aStringArray107.length; i_4_++) {
					if (Class8.aStringArray107[i_4_].indexOf("--> ") != -1
							&& ++i_3_ == Class8.anInt105 * -1217082313) {
						Class8.aString101 = (Class8.aStringArray107[i_4_]
								.substring(Class8.aStringArray107[i_4_]
										.indexOf('>') + 2));
						break;
					}
				}
