...
public class Class297 {
	...
	Class297(byte i, int i_0_, int i_1_, int i_2_, int i_3_, int i_4_,
			int i_5_, Class365_Sub1 class365_sub1) {
		...
		if (class365_sub1 != null
				&& class365_sub1 instanceof Class365_Sub1_Sub1) {
	...
	public static int method2836(byte i) {
		try {
			...
			throw Class346.method4175(runtimeexception, new StringBuilder()
					.append("mj.r(").append(')').toString());
		}
	}

	static final void decodePlayerMasks(RsBitsBuffer stream, int playerIndex,
			Player player, int mask) {
		try {
			...
			if ((mask & 0x4) != 0) { // gfx 1
				...
				player.sendGraphics(i_10_, i_11_, i_13_, i_14_, bool, 0,
						831413227);
			}
			if ((mask & 0x40) != 0) { //face direction
				player.anInt10221 = stream.readUnsignedShort()
						* -1530078181;
			...
			if (0 != (mask & 0x8000)) { // gfx 2
				...
				player.sendGraphics(i_15_, i_16_, i_18_, i_19_, bool, 1,
						-589042001);
			}
			if (0 != (mask & 0x8)) { // hits
				...
				if (i_20_ > 0) {
					for (int i_21_ = 0; i_21_ < i_20_; i_21_++) {
						...
						player.method4420(i_25_, i_23_, i_22_, i_24_,
								443738891 * client.anInt8884, i_26_, i_27_,
								(byte) 121);
					}
				}
			}
			if (0 != (mask & 0x400000)) { // gfx 3
				...
				player.sendGraphics(i_28_, i_29_, i_31_, i_32_, bool, 2,
						1066291785);
			}
			if (0 != (mask & 0x800000)) { // gfx 4
				...
				player.sendGraphics(i_33_, i_34_, i_36_, i_37_, bool, 3,
						305107810);
			}
			...
			if ((mask & 0x20000) != 0) {
				...
				if ((i_40_ & 0x1) != 0)
					Class242_Sub1.method2282(2, i_40_,
							player.method4470(true, -2017419584),
							player.method4475(false, (byte) -87),
							player.aString10200, string, 252423564);
			...
			if ((mask & 0x2000) != 0) {
				...
				player.anInt10086 = (1208636921 * client.anInt8884
						+ -1132907677 * player.anInt10087 + 1195117671 * player.anInt10088);
			}
			...
			if ((mask & 0x80) != 0) // movement type
				Class10.playersMovementTypes[playerIndex] = stream
						.read128Byte(44745097);
			if (0 != (mask & 0x4000)) {
				...
				Class298_Sub37_Sub7.method3428(player, is, is_43_, is_44_,
						1644064563);
			}
			...
			if (0 != (mask & 0x100)) { // forcetalk
				...
				if (player == Class287.myPlayer)
					Class242_Sub1.method2282(2, 0,
							player.method4470(true, -1880046656),
							player.method4475(false, (byte) -103),
							player.aString10200, string, -588173673);
			...
			if ((mask & 0x200000) != 0) {
				...
				player.aByte10130 = (byte) stream
						.readUnsignedByteC((short) -10460);
				player.anInt10105 = ((443738891 * client.anInt8884 + stream
						.readUnsignedShort()) * 1828453179);
				player.anInt10106 = ((client.anInt8884 * 443738891 + stream
						.readUnsignedShort()) * -473408095);
			}
			if ((mask & 0x80000) != 0) {
			...
			if (player.aBoolean10217) {
				if (movementType == 127)
					player.method4472(659113193 * player.anInt10218, -504032157
							* player.anInt10219, -2047021901);
				else {
					byte i_61_;
					...
					player.method4471(659113193 * player.anInt10218, -504032157
							* player.anInt10219, i_61_, -2128158189);
				}
			}
		} catch (RuntimeException runtimeexception) {
			throw Class346.method4175(runtimeexception, new StringBuilder()
					.append("mj.u(").append(')').toString());
		}
	}

	static final void method2838(Class403 class403, int i) {
		try {
			((Class403) class403).anIntArray5244[((((Class403) class403).anInt5239 += -391880689) * 681479919 - 1)] = Class406.aBoolean5274 ? 1
					: 0;
		} catch (RuntimeException runtimeexception) {
			throw Class346.method4175(runtimeexception, new StringBuilder()
					.append("mj.amu(").append(')').toString());
		}
	}
}
