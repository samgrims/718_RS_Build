	...
	public static boolean method762(String string, int i, String string_0_,
		...
		try {
			if (i == 0) {
				...
				try {
					...
					Runtime.getRuntime().exec(
							new StringBuilder().append("cmd /c start \"j\" \"")
			...
			if (1 == i) {
				boolean bool;
				try {
					Object object = Class466.method6022(ClientScriptMap.anApplet6044,
							string_0_,
							(new Object[] { new URL(ClientScriptMap.anApplet6044
									.getCodeBase(), string).toString() }),
							(byte) 44);
					bool = null != object;
				} catch (Throwable throwable) {
					return false;
